...
In [part 1](/tensorflow/tf.function/2019/03/21/dissecting-tf-function-part-1/) we learned how to convert a 1.x code to its eager version, the eager version to its graph representation and concluded analyzing the problems to face when working with functions that create a state.
...
At the line (rewritten and formatted for clarity)
```python
...
The [`converted_call`](https://github.com/tensorflow/tensorflow/blob/56c8527fa73f694b76963dbb28a9d011d233086f/tensorflow/python/autograph/impl/api.py#L206) function compiles a function call: this function has all the information required to convert and execute a function call; its signature is `(f, owner, options, args, kwargs)` where
